...
실수 $a$ 에 대해 이원수 (dual number) $(a,\, \epsilon)_D$ 은 $a$ 와 $a$ 값에 대한 오차 $\epsilon$ 을 의미하며 사칙연산을 포함한 연산에 대해 $\epsilon$ 에 대한 $1$ 차만을 허용한다. 즉 $\epsilon^2 =0$ 으로 간주한다. 그리고 다음 규칙을 따른다.
$$
\begin{aligned}
c(a,\, \epsilon)_D &= (ca,\, c\epsilon)_D, \\[0.3em]
1/(a,\, \epsilon)_D &= (1/a,\, -\epsilon/a^2)_D,\\[0.3em]
(a,\, \epsilon_1)_D + (b,\,\epsilon_2)_D &= (a+b,\, \epsilon_1+\epsilon_2)_D, \\[0.3em]
(a,\, \epsilon_1)_D - (b,\,\epsilon_2)_D &= (a-b,\, \epsilon_1-\epsilon_2)_D, \\[0.3em]
(a,\, \epsilon_1)_D \times (b,\,\epsilon_2)_D &= (ab,\, b\epsilon_1+a\epsilon_2)_D.
\end{aligned}
$$ {#eq-derivative_epsilon_rules}
위 규칙은 함수 $f(x)$ 에 대한 $x=a$ 에서의 테일러 1차 전개 규칙과 동일하다. 예를 들어

$$
f(a+h) \approx f(a)+f'(a)h,\qquad g(x+h) \approx g(a)+g'(a)h
$$

에 대해 $\epsilon_1 = f'(a)h,\, \epsilon_2=g'(a)h$ 라 놓으면 $f(a+h) = f(a)+\epsilon_1$, $g(a+h)= g(a)+\epsilon_2$ 이며

$$
(fg)(a+h) \approx (fg)(a) + f(a)g'(a)h + g(a)f'(a)h = f(a)g(a) + f(a)\epsilon_2 + g(a)\epsilon_1
$$

으로 @eq-derivative_epsilon_rules 의 마지막 식과 동일하다. 

</br>

#### **미분가능한 함수와 이원수**

이제 실수를 이원수 $(a,\, \epsilon)_D$ 로 표현한다고 하자. 그리고 실수에 대한 함수 미분가능한 함수 $f(x)$ 는 실수, 혹은 이원수 입력에 대해 다음과 이원수를 다음과 같이 반환한다고 하자.

$$
\begin{aligned}
f((a,\, \epsilon)_D) &= (f(a),\, \epsilon f'(a))_D, \\[0.3em]
f(a) &= f((a,\, 1)_D) = (f(a),\, f'(a))_D
\end{aligned}
$$

이제 함수의 스칼라곱과 사칙 연산에 대해 함수에 대한 이원수 표현이 일관된다는 것을 아래와 같이 보일 수 있다.


$$
\begin{aligned}
c(f(a)) &= c(f(a),\, f'(a))_D = (cf)(a), \\[0.3em]
f(a) + g(a) &= (f(a),\, f'(a)) + (g(a),\, g'(a))_D = (f(a)+g(a) ,\, f'(a)+g'(a))_D = (f+g)(a), \\[0.3em]
f(a)g(a) &= (f(a),\, f'(a)) \times (g(a),\, g'(a))_D = ((fg)(a),\, f'(a)g(a)+f(a)g'(a))_D = (fg)(a), \\[0.3em]
f(a)/g(a) &= \left(f(a),\, f'(a) \times (1/g(a),\, -g'(a)/(g(a))^2)\right)_D \\[0.3em]
& = \left((f/g)(a),\, (f'(a)g(a) - f(a)g'(a))/(g(a))^2\right)_D = (f/g)(a).  
 \end{aligned}
$$

이제 함수의 합성에 대해 이원수 표현이 일관된다는 것을 보이자. 우선 이원수 표현이 함수 합성에 대해 일관되려면 아래와 같은 등식이 성립해야 한다.

$$
(f\circ g)(a) = ((f\circ g)(a),\, (f\circ g)'(a))_D = ((f \circ g)(a),\,  (f\circ g)'(a))_D = ((f \circ g)(a),\,  f'(g(a))g'(a))_D
$$ {#eq-derivative_autodiff_composition_1}
...
$$
\begin{aligned}
(f\circ g)(a) &= f(g(a)) = f((g(a),\, g'(a))_D) = (f(g(a)),\, f'(g(a))g'(a))_D.
\end{aligned}
$${#eq-derivative_autodiff_composition_2}


즉 이원수를 도입하고, 이원수와 이원수에 대한 함수를 위와 같이 정의하면 함수의 사칙연산과 함성에 대한 연산을 수행 할 수 있다.

</br>

### 1차원 변와 함수의 자동미분의 구현

아래의 julia 코드는 이원수와 이원수에 대한 몇몇 함수의 구현이다. 이원수는 `DNumber` 라는 복합 타입으로 구현되었으며 `v` 와 `e` 라는 멤버를 가지는데 각각 값과 $\epsilon$ 을 의미한다. 그리고 몇몇 중요 함수를 dispatch 하여 이원수에 대해 사용 가능하도록 하였다.
...
위의 코드는 자동 미분이 이루어지는 방식을 설명하기 위한 것이며 앞으로 사용하지 않는다. 

</br>

### 다변수 함수에 대한 이원수 와 함수

$f:\R^n \to \R$ 을 다룰 경우 $(\bf{x},\, \bf{\epsilon})_D$ 에 대해 스칼라곱과 합, 그리고 함수에 대한 값을 다음과 같이 정의한다.

$$
\begin{aligned}
c(\bf{x},\, \bf{\epsilon})_D &= (c\bf{x},\, c\bf{\epsilon})_D, \\[0.3em]
(\bf{x},\, \bf{\epsilon})_D + (\bf{y},\, \bf{\delta})_D &= (\bf{x}+\bf{y},\, \bf{\epsilon}+ \bf{\delta})_D, \\[0.3em]
\end{aligned}
$$

$$
f((\bf{a},\, \bf{\epsilon})_D) = (f(\bf{a}),\,  \nabla f(\bf{a})\bf{\cdot \epsilon})_D
$$
...
즉 후진법은 전진하면서 함수값을 계산하고 후진하면서 앞서 계산한 함수값을 이용하여 편미분을 계산한다. 일반적으로 $F:\R^n \to \R^m$ 에 대해 $m>n$ 이면 전진법이 유리하며 $n>m$ 이면 후진법이 유리하다는 것이 잘 알려져 있으며 $n$ 이 $m$ 보다 더 클수록 더 유리하다는 것이 알려져 있다. 인공신경망에서의 오차함수 계산이 전형적으로 이런 경우이며 따라서 인공신경망에서의 backpropagation 에서는 후진법이 사용된다.
...
$H(\bf{x})= \begin{bmatrix} x_1 x_2 \\ x_1 + x_2\end{bmatrix}$ 에 대한 $\bf{J}_H(1.0, 2.0)$ 은 다음과 같이 계산한다.
...
$f(\bf{x})=x_1 \sin (\pi x_2) + 2x_2^3$ 에 대한 헤시안 $\bf{H}_f(1,1)$ 은 다음과 같이 계산한다.


```{.julia filename="REPL In"}
h(x::Vector)=x[1]*sin(π*x[2]) + 2*x[2]^3
ForwardDiff.hessian(h, [1, 1])
```
